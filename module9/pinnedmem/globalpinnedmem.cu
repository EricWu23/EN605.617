...
__host__ void execute_gpu_pinnedmem_arrayAdd(int numBlocks,int blockSize,int *const gpu_array0,int * const gpu_array1,int* const gpu_arrayresult,int* const h_array_res,int* const h_array0,int* const h_array1){ 	
	...
	const dim3 threads_layout(WARP1,blockSize/WARP1); // there are multiple ways of layout to achieve blocksize. I choose to fix the  blockDim.x as the WARP1 size
	...
	cudaMemcpy( gpu_array0,h_array0 , size_in_bytes, cudaMemcpyHostToDevice );
    cudaMemcpy( gpu_array1,h_array1 , size_in_bytes, cudaMemcpyHostToDevice ); 
	arrayAdd<<<blocks_layout,threads_layout>>>(gpu_array0,gpu_array1,gpu_arrayresult); // kernel call to add two 2-D arrays 
	cudaMemcpy(h_array_res, gpu_arrayresult, size_in_bytes, cudaMemcpyDeviceToHost); // memcopy from gpu to cpu
...
__host__ void execute_gpu_pinnedmem_arraySubtract(int numBlocks,int blockSize,int *const gpu_array0,int * const gpu_array1,int* const gpu_arrayresult,int* const h_array_res,int* const h_array0,int* const h_array1){
	int totalThreads=numBlocks*blockSize;
	int cpu_arr_size_y=1;//row
	int cpu_arr_size_x=totalThreads;//column
	int size_in_bytes = cpu_arr_size_x* cpu_arr_size_y* sizeof(int);
...
__host__ void execute_gpu_pinnedmem_arrayMult(int numBlocks,int blockSize,int *const gpu_array0,int * const gpu_array1,int* const gpu_arrayresult,int* const h_array_res,int* const h_array0,int* const h_array1){
    int totalThreads=numBlocks*blockSize;
	int cpu_arr_size_y=1;//row
	int cpu_arr_size_x=totalThreads;//column
	int size_in_bytes = cpu_arr_size_x* cpu_arr_size_y* sizeof(int);
...
__host__ void execute_gpu_pinnedmem_arrayMod(int numBlocks,int blockSize,int *const gpu_array0,int * const gpu_array1,int* const gpu_arrayresult,int* const h_array_res,int* const h_array0,int* const h_array1){
    int totalThreads=numBlocks*blockSize;
	int cpu_arr_size_y=1;//row
	int cpu_arr_size_x=totalThreads;//column
	int size_in_bytes = cpu_arr_size_x* cpu_arr_size_y* sizeof(int);
...
void execute_gpu_pinnedmem_test(int numBlocks, int blockSize){
	printf("Unit Test 1: Simple Math Operations with global device memory and Host pinned memory\n");
	printf("-------------------------------------------------------------------------------------\n");
	int totalThreads=numBlocks*blockSize;
	int cpu_arr_size_y=1;//row
	int cpu_arr_size_x=totalThreads;//column
	...
	for(int kernel=0; kernel<4; kernel++){//Execute 4 simple math operation
      switch(kernel){
            case 0:{ execute_gpu_pinnedmem_arrayAdd(numBlocks,blockSize,gpu_array0,gpu_array1,gpu_arrayresult,h_array_res,h_array0,h_array1);
                    } break;                                                                                     
            case 1:{execute_gpu_pinnedmem_arraySubtract(numBlocks,blockSize,gpu_array0,gpu_array1,gpu_arrayresult,h_array_res,h_array0,h_array1);
                   }break;                                     
           case 2:{execute_gpu_pinnedmem_arrayMult(numBlocks,blockSize,gpu_array0,gpu_array1,gpu_arrayresult,h_array_res,h_array0,h_array1);
                   }break;                                                                 
           case 3:{ execute_gpu_pinnedmem_arrayMod(numBlocks,blockSize,gpu_array0,gpu_array1,gpu_arrayresult,h_array_res,h_array0,h_array1);
                   }break;                                                                   
